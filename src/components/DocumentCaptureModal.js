...
async function deleteTemporaryDocumentFile(uri) {
  if (!uri || !String(uri).startsWith("file:")) return;

  try {
    await LegacyFileSystem.deleteAsync(uri, { idempotent: true });
  } catch {
    // Temporary-file cleanup must never block cancellation or selection.
  }
}
...
export default function DocumentCaptureModal({
  ...
  useEffect(
    () => () => {
      ...
      if (pendingPhoto?.uri && pendingPhoto.uri !== transferredUriRef.current) {
        deleteTemporaryDocumentFile(pendingPhoto.uri);
      }
    },
    []
  );
  ...
  const closeCapture = async () => {
    ...
    await deleteTemporaryDocumentFile(photo?.uri);
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      setCameraError(error?.message || "Unable to capture the document. Please try again.");
    } finally {
      setCapturing(false);
    }
  };
  ...
  const usePhoto = async () => {
    ...
    try {
      ...
      setCameraError(error?.message || "Unable to use this photo. Please try again.");
    } finally {
      setUsingPhoto(false);
    }
  };

  const instruction = `Position the ${side} of your ID inside the frame.`;
      ...
      return (
        <View style={styles.centeredState}>
          ...
          <Text style={styles.stateText}>
            FleetX needs camera access to photograph your verification document.
          </Text>
          <TouchableOpacity
      ...
      return (
        <View style={styles.previewContainer}>
          ...
          <View style={styles.previewContent}>
            <Text style={styles.previewTitle}>Review your {side} photo</Text>
            <Text style={styles.previewInstruction}>
              Make sure the entire document and all four corners are clear and readable.
            </Text>
    ...
    return (
      <View style={styles.cameraContainer}>
        <CameraView
          ...
          onMountError={(event) =>
            setCameraError(event?.message || "Unable to start the rear camera.")
          }
        />
        ...
        <View style={styles.captureInstructions} pointerEvents="none">
          <Text style={styles.captureTitle}>{instruction}</Text>
          <Text style={styles.captureSubtitle}>
            Keep all corners visible and avoid blur or glare.
          </Text>
        </View>
        ...
        <View style={styles.captureControls}>
          <TouchableOpacity
            accessibilityLabel={`Capture ${side} of ${documentLabel}`}
            style={[styles.shutterOuter, (!cameraReady || capturing) && styles.buttonDisabled]}
    ...
    >
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{documentLabel} · {side === "front" ? "Front" : "Back"}</Text>
...
const styles = StyleSheet.create({
  ...
  body: { flex: 1, backgroundColor: "#05080d" },
  cameraContainer: { flex: 1, overflow: "hidden" },
  guideOverlay: { ...StyleSheet.absoluteFillObject },
  topShade: { flex: 1, backgroundColor: "rgba(3, 7, 18, 0.58)" },
  guideRow: { height: 220, flexDirection: "row" },
  sideShade: { flex: 1, backgroundColor: "rgba(3, 7, 18, 0.58)" },
  documentFrame: {
    width: "86%",
    maxWidth: 360,
    height: 220,
  ...
  bottomShade: { flex: 1, backgroundColor: "rgba(3, 7, 18, 0.58)" },
  captureInstructions: {
    position: "absolute",
    left: 24,
    right: 24,
  ...
  captureTitle: { color: "#ffffff", fontSize: 18, fontWeight: "700", textAlign: "center" },
  captureSubtitle: {
    color: "#dbe4f0",
    fontSize: 14,
    lineHeight: 20,
  ...
  captureControls: { position: "absolute", left: 0, right: 0, bottom: 30, alignItems: "center" },
  shutterOuter: {
    width: 76,
    height: 76,
    borderRadius: 38,
  ...
  centeredState: { flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 30 },
  stateTitle: { color: "#ffffff", fontSize: 20, fontWeight: "700", marginTop: 18, textAlign: "center" },
  stateText: {
    color: "#cbd5e1",
    fontSize: 15,
    lineHeight: 22,
  ...
  previewContainer: { flex: 1, backgroundColor: "#05080d" },
  ...
  usePhotoButton: { flex: 1, minWidth: 0 },
  errorText: { color: "#fecaca", fontSize: 13, textAlign: "center", marginTop: 10 },
});
